...
#####################################################################
### Identifying cell-type composition changes, on "pooled" experiment
### Chengxiang Qiu
### Feb-20, 2025
...
cell_gRNA = readRDS(paste0(work_path, "/analysis/monoclonal_EB_TF_screen/cell_to_mEB/cell_clonotype_gRNA_rep2.rds"))
...
### out of 157 clonotypes, 3 clonotypes are excluded due to limited gRNA UMIs, for the left 154 clonotypes,
### 139 are assigned with unique gRNAs, 15 are assigned with double gRNAs
### 140 are assigned with unique target, 14 are assigned with double targets
cell_gRNA = rbind(cell_gRNA_uniq, cell_gRNA_mult)
cell_target = cell_gRNA %>% select(clonotype_id, target) %>% unique()

### maybe only using colontype with unique barcode
### cell_target = rbind(cell_gRNA_uniq, subset(cell_gRNA_mult, clonotype_id == "clonotype_113")) %>% 
###    select(clonotype_id, target) %>% unique()
...
### 71 samples, with 8 are double assigned (79 combinations in total)
...
for(i in target_list){
    ...
    try(ggplot() +
            ...
            ggsave(paste0(work_path, "/analysis/monoclonal_EB_TF_screen/celltype_changes/rep2/UMAP_rep2_",  i, ".png"),
                   dpi = 300,
                   height  = 5, 
                   width = 5), silent = TRUE)
}
...
df = cell_BC_assigned %>% 
    ...
    left_join(cell_target, by = "clonotype_id", relationship = "many-to-many") %>%
    filter(!is.na(target)) %>%
    group_by(target, celltype) %>%
    tally()
df$target = factor(df$target, levels = c("T","Lhx1","Hand1","Gata6","Hes5","Six3","Lmo2","Hhex","NONTARGETING"))
...
p = df %>%
    ...
    ggsave(paste0(work_path, "/analysis/monoclonal_EB_TF_screen/celltype_changes/rep2/celltype_composition_target.pdf"),
           dpi = 300,
           height  = 5, 
           width = 5)
...
pd_sub = pd_sub %>% select(cell_id, experiment_id, UMAP_1, UMAP_2, celltype, clonotype_id) %>% 
    left_join(cell_target, by = "clonotype_id", relationship = "many-to-many") %>% filter(!is.na(target)) %>% as.data.frame()
rownames(pd_sub) = paste0("cell_", 1:nrow(pd_sub))

count = GetAssayData(obj, slot = "counts")[,as.vector(pd_sub$cell_id)]
colnames(count) = paste0("cell_", 1:nrow(pd_sub))
...
### combine clonotype_id + target, with 79 combinations
cds$clonotype_id_target = paste0(cds$clonotype_id, "_", cds$target)

ccs = new_cell_count_set(cds, 
                         sample_group = "clonotype_id_target", 
                         cell_group = "celltype")

### Gata6        Hand1         Hes5         Hhex         Lhx1         Lmo2
###    16           19            6           10            8            6
### NONTARGETING         Six3            T
###            5            4            5
...
res = NULL
for (target_i in c("T","Lhx1","Hand1","Gata6","Hes5","Six3","Lmo2","Hhex")){
    print(target_i)
...
saveRDS(res, paste0(work_path, "/analysis/monoclonal_EB_TF_screen/celltype_changes/rep2/hooke_result.rds"))

write.table(res, paste0(work_path, "/analysis/monoclonal_EB_TF_screen/celltype_changes/rep2/hooke_result.txt"), row.names=F, quote=F, sep="\t")
...
dat = dat[c("T","Lhx1","Hand1","Gata6","Hes5","Six3","Lmo2","Hhex"),
...
pdf(paste0(work_path, "/analysis/monoclonal_EB_TF_screen/celltype_changes/rep2/hooke_heatmap.pdf"), 8, 5)
...
res = readRDS(paste0(work_path, "/analysis/monoclonal_EB_TF_screen/celltype_changes/rep2/hooke_result.rds"))
...
ggsave(paste0(work_path, "/hooke_volcano_plot_2_label_gene.pdf"), p, width = 5.5, height = 3)
...
p = ggplot() +
    ...
    theme(plot.title = element_text(hjust = 0.5)) +
    theme(axis.text.x = element_text(color="black"), axis.text.y = element_text(color="black")) 
ggsave(paste0(work_path, "/hooke_volcano_plot_2_label_cell_group.pdf"), p, width = 5.5, height = 3)


##################################
### Step-5: comparing with arrayed

res_1 = readRDS(paste0(work_path, "/analysis/monoclonal_EB_TF_screen/celltype_changes/rep1/hooke_result.rds"))
res_2 = readRDS(paste0(work_path, "/analysis/monoclonal_EB_TF_screen/celltype_changes/rep2/hooke_result.rds"))

df = res_1 %>% 
    select(cell_group, target_y, delta_log_abund_rep1 = delta_log_abund, fdr_rep1 = fdr) %>%
    left_join(res_2 %>% select(cell_group, target_y, delta_log_abund_rep2 = delta_log_abund, fdr_rep2 = fdr), by = c("cell_group", "target_y")) %>%
    filter(!is.na(fdr_rep2)) %>% as.data.frame()

cor.test(df$delta_log_abund_rep1, df$delta_log_abund_rep2, method = "pearson")

df %>% 
    ggplot() + 
    geom_point(data = df, aes(delta_log_abund_rep1, delta_log_abund_rep2), color = "grey") + 
    geom_point(data = subset(df, fdr_rep1 < 0.1), aes(delta_log_abund_rep1, delta_log_abund_rep2), color = "red") + 
    labs(x="", y="", title="") +
    theme_classic(base_size = 10) +
    theme(legend.position="none") +
    theme(plot.title = element_text(hjust = 0.5)) +
    theme(axis.text.x = element_text(color="black"), axis.text.y = element_text(color="black")) +
    ggsave(paste0(work_path, "/analysis/monoclonal_EB_TF_screen/celltype_changes/rep2/scatter_plot_compare_rep1.pdf"),
           dpi = 300,
           height  = 5, 
           width = 5)
...
res_sig = res_1 %>% filter(fdr < 0.1, target_y != "Carm1")
...
for(i in 1:nrow(res_sig)){
    ...
    try(ggplot(df, aes(group, frac, fill = group)) + geom_boxplot(outlier.shape = NA) +
            ...
            theme(plot.title = element_text(hjust = 0.5)) +
            theme(axis.text.x = element_text(color="black"), axis.text.y = element_text(color="black")) +
            ggsave(paste0(work_path, "/analysis/monoclonal_EB_TF_screen/celltype_changes/rep2/boxplot_", target_i, "_", gsub(" ","_",celltype_i), ".pdf"),
                   height  = 5, 
                   width = 3), silent = T)
...
x_2 = as.vector(cell_freq[rownames(cell_freq) == "Lateral plate mesoderm",
                          ccs$target == "NONTARGETING"])
wilcox.test(x_1, x_2) ### p = 0.0001882



####################################################################################
### Step-6: making a big heatmap, each column is a clonotype, each row is a celltype 

cell_BC_assigned = readRDS(paste0(work_path, "/analysis/monoclonal_EB_TF_screen/cell_to_mEB/cell_clonotype_rep2.rds"))
pd = readRDS(paste0(work_path, "/analysis/monoclonal_EB_TF_screen/obj_aligned_pd.rds"))

cell_target$clonotype_id_dup = paste0("clonotype_", 1:nrow(cell_target))
...
dat = pd_sub %>% group_by(clonotype_id_dup, celltype) %>% tally() %>%
    dcast(celltype~clonotype_id_dup, fill = 0)
...
cell_target_x = data.frame(clonotype_id_dup = clonotypeID) %>%
    left_join(cell_target, by = "clonotype_id_dup")

library("gplots")
library(RColorBrewer)
Colors=rev(brewer.pal(11,"Spectral"))
Colors=colorRampPalette(Colors)(120)
pdf(paste0(work_path, "/analysis/monoclonal_EB_TF_screen/celltype_changes/rep2/clonotype_celltype_composition_heatmap_rep2.pdf"), 8, 5)
...
###############################################
### Step-7: making UMAP on individual clonotype
...
for(target_i in target_list){
    ...
    try(ggplot() +
            ...
            ggsave(paste0(work_path, "/analysis/monoclonal_EB_TF_screen/celltype_changes/rep2/UMAP_clonotype_rep2_", target_i, ".png"),
                   dpi = 300,
                   height  = row_num, 
                   width = 10), silent = T)
...
df %>% 
    ...
    theme(plot.title = element_text(hjust = 0.5)) +
    theme(axis.text.x = element_text(color="black"), axis.text.y = element_text(color="black")) +
    ggsave(paste0(work_path, "/analysis/monoclonal_EB_TF_screen/celltype_changes/rep2/gene_exp_compare.pdf"), 
           height  = 5, 
           width = 5)
...
for(i in 1:length(gene_list)){
    ...
    try(ggplot(df, aes(target, exp, fill = target)) + geom_boxplot(outlier.shape = NA) +
            ...
            theme(plot.title = element_text(hjust = 0.5)) +
            theme(axis.text.x = element_text(color="black"), axis.text.y = element_text(color="black")) +
            ggsave(paste0(work_path, "/analysis/monoclonal_EB_TF_screen/celltype_changes/rep2/boxplot_gene_exp_", gene_i, ".pdf"),
                   height  = 5, 
                   width = 3), silent = T)
}
...
for(i in 1:length(gene_list)){
    ...
    try(ggplot(df, aes(target, exp, fill = target)) + geom_boxplot(outlier.shape = NA) +
            ...
            ggsave(paste0(work_path, "/analysis/monoclonal_EB_TF_screen/celltype_changes/rep2/boxplot_gene_exp_by_celltype_", gene_i, ".pdf"),
                   height  = 3, 
                   width = 12), silent = T)
}
...
############################################################################################
### Step-9: scatterplot of TF expression vs cell type proportion per EB (for each cell type)
...
for(i in 1:length(gene_list)){
    ...
    try(ggplot(df, aes(mean_exp, frac, color = clonotype_id)) + geom_point() +
            ...
            theme(plot.title = element_text(hjust = 0.5)) +
            theme(axis.text.x = element_text(color="black", angle = 90), axis.text.y = element_text(color="black")) +
            ggsave(paste0(work_path, "/analysis/monoclonal_EB_TF_screen/celltype_changes/rep2/TF_exp_vs_celltype_frac_", gene_i, ".pdf"),
                   height  = 3, 
                   width = 20), silent = T)
