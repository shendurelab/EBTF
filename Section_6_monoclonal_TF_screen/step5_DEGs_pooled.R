...
##########################################################################################
### Performing DEGs between Hand1 and NTCs at pseudobulk level, in the "pooled" experiment
### Chengxiang Qiu
### Feb-20, 2025
...
### 71 samples, with 8 are double assigned (79 in total)
...
saveRDS(list(count_aggr, cell_target), paste0(work_path, "/analysis/monoclonal_EB_TF_screen/DEG/count_aggr_2.rds"))
...
dat_tmp = readRDS(paste0(work_path, "/analysis/monoclonal_EB_TF_screen/DEG/count_aggr_2.rds"))
...
metadata = cell_target[keep,]; rownames(metadata) = as.vector(metadata$clonotype_id)
count_data = count_aggr[,as.vector(metadata$clonotype_id)]
...
write.table(results_sig, paste0(work_path, "/analysis/monoclonal_EB_TF_screen/DEG/Hand1_vs_NTC_2.txt"), row.names=F, quote=F, sep="\t")


######################################
### Step-4: combining 1 and 2 together

res1 = read.table(paste0(work_path, "/analysis/monoclonal_EB_TF_screen/DEG/Hand1_vs_NTC_1.txt"), header=T, as.is=T)
res2 = read.table(paste0(work_path, "/analysis/monoclonal_EB_TF_screen/DEG/Hand1_vs_NTC_2.txt"), header=T, as.is=T)

res = res1 %>% select(gene_short_name, log2FoldChange_exp1 = log2FoldChange, padj_exp1 = padj) %>%
    full_join(res2 %>% select(log2FoldChange_exp2 = log2FoldChange, padj_exp2 = padj, gene_short_name), by = "gene_short_name")

write.table(res, paste0(work_path, "/analysis/monoclonal_EB_TF_screen/DEG/Hand1_vs_NTC_1_and_2.txt"), row.names=F, quote=F, sep="\t")



##########################################################################
### Step-5: performing a single-cell level analysis (w/o monoclonal types)
...
result_sig = read.table(paste0(work_path, "/analysis/monoclonal_EB_TF_screen/DEG/Hand1_vs_NTC_2.txt"), header=T)
...
ggsave("~/share/EBTF_experiment2_Venn.pdf", p1+p2, width = 8, height = 4)




################################################################################
### Step-6: performing GO analysis on shared genes vs. only found in single-cell
...
res$experiment = "experiment_2"

write.table(res, paste0(work_path, "/analysis/monoclonal_EB_TF_screen/DEG/DEG_GO_analysis_2.txt"), row.names=F, sep="\t", quote=F)
