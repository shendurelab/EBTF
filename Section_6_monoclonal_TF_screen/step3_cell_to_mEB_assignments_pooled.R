...
############################################################################
### Assiging cells to each mEBs based on the barcode, on "pooled" experiment
### Chengxiang Qiu
### Feb-20, 2025
...
pd = readRDS(paste0(work_path, "/analysis/monoclonal_EB_TF_screen/obj_processed_pd.rds"))

run_list = paste0("clonal_EBTF_", c(13:16))
...
for(run_id in run_list){
    print(run_id)
    bc_matrix_table = read.table(paste0(work_path, "/data/monoclonal_EB_TF_screen/clonal_EBTF_13_thru_16_get_bc.txt"), header=T, as.is=T)
...
try(ggplot() +
        ...
        ggsave(paste0(work_path, "/analysis/monoclonal_EB_TF_screen/cell_to_mEB/BC_2D_hist_rep2.pdf"),
               height  = 3, 
               width = 5), silent = TRUE)


### After checking the 2D-hist, I decided to use log2_read_count >= 15 as cutoff to filter BCs
### n = 293 BCs are retained
...
saveRDS(bc_matrix, paste0(work_path, "/analysis/monoclonal_EB_TF_screen/cell_to_mEB/BC_matrix_rep2.rds"))
saveRDS(gRNA_matrix, paste0(work_path, "/analysis/monoclonal_EB_TF_screen/cell_to_mEB/gRNA_matrix_rep2.rds"))
...
ggplot(data = subset(cell_BC, UMI_count > 0), aes(log2(UMI_count))) + geom_histogram(bins = 30) + 
    ...
    ggsave(paste0(work_path, "/analysis/monoclonal_EB_TF_screen/cell_to_mEB/hist_umi_per_BC_per_cell_rep2.pdf"),
           height  = 4, 
           width = 4)

ggplot(data = subset(cell_BC, UMI_count > 0), aes(pct)) + geom_histogram(bins = 30) + 
    geom_vline(xintercept = 30) + theme_classic(base_size = 10) +
    ggsave(paste0(work_path, "/analysis/monoclonal_EB_TF_screen/cell_to_mEB/hist_pct_umi_per_BC_per_cell_rep2.pdf"),
           height  = 4, 
           width = 4)
...
saveRDS(cell_BC_assigned, paste0(work_path, "/analysis/monoclonal_EB_TF_screen/cell_to_mEB/cell_clonotype_rep2.rds"))
print(nrow(cell_BC_assigned)/sum(pd$experiment_id %in% paste0("monoclonal_EB_TF_screen_clonal_EBTF_", c(13:16))))
### Finally, 59.1% of cells with transcriptome have been assigned to a distinct clonotype in rep2
...
pd = pd %>% left_join(cell_BC_assigned[,c("clonotype_id", "cell_id")]) ### 19211 are assigned
...
try(ggplot() +
        ...
        theme(legend.position="none") +
        theme(plot.title = element_text(hjust = 0.5)) +
        ggsave(paste0(work_path, "/analysis/monoclonal_EB_TF_screen/cell_to_mEB/UMAP_clonotype_before_filtering_rep2_nolabel.png"),
               dpi = 300,
               height  = 5, 
               width = 5), silent = TRUE)
...
for(i in clonotype_list){
    ...
    if(sum(colSums(gRNA_matrix_i) >= 10) >= 10){
        gRNA_matrix_i = gRNA_matrix_i[,colSums(gRNA_matrix_i) >= 10,drop=FALSE]
...
saveRDS(cell_gRNA, paste0(work_path, "/analysis/monoclonal_EB_TF_screen/cell_to_mEB/cell_clonotype_gRNA_rep2.rds"))

### of note, three clonotype were not assigned with gRNAs, because they didn't pass this step:
### sum(colSums(gRNA_matrix_i) >= 10) >= 10
### clonotype_115, clonotype_122, clonotype_151
...
ggplot(df, aes(x = order, y = pct_cum, color = clonotype_id)) + 
    ...
    theme(plot.title = element_text(hjust = 0.5)) +
    theme(axis.text.x = element_text(color="black"), axis.text.y = element_text(color="black")) +
    ggsave(paste0(work_path, "/analysis/monoclonal_EB_TF_screen/cell_to_mEB/cell_clonotype_gRNA_rep2_line_plot.pdf"),
           dpi = 300,
           height  = 5, 
           width = 5)
...
sum(dat$pct >= 85 & dat$gRNA_pct >= 85)/nrow(dat) ### 84.4%
...
dat %>% 
    ...
    theme(plot.title = element_text(hjust = 0.5)) +
    theme(axis.text.x = element_text(color="black"), axis.text.y = element_text(color="black")) +
    ggsave("~/share/UMIs_of_oBC_gRNA_pairs.pdf", width=3, height=3)
