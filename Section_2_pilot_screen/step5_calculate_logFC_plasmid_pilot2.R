...
plasmid_i_file = "CRISPRi_pilot_sgRNAs_from_plasmid.txt"
plasmid_cut_file = "CRISPRcut_pilot_sgRNAs_from_plasmid.txt"

ESC_i_file = "CRISPRi_pilot2_sgRNAs_from_ESC.txt"
ESC_cut_file = "CRISPRcut_pilot2_sgRNAs_from_ESC.txt"
...
ESC_cut = read.table(ESC_cut_file, as.is=T)
plasmid_cut = plasmid_cut %>% left_join(ESC_cut, by = "sgRNAs") %>% rename(ES_tr_cut = ES_tr_cut_pilot) %>% as.data.frame()
...
ESC_i = read.table(ESC_i_file, as.is=T)
plasmid_i = plasmid_i %>% left_join(ESC_i, by = "sgRNAs") %>% rename(EB_ind_i = ES_tr_i_pilot) %>% as.data.frame()
...
for(run_id in run_list){
    ...
    if(run_id %in% c("rep2_EB21_CRISPRcut_sorted_gRNA","rep2_EB21_CRISPRcut_sorted_gRNA")){
    ...
    if(run_id %in% c("rep2_EB21_CRISPRcut_sorted_gRNA","rep2_EB21_CRISPRcut_sorted_gRNA")){
        df1 = dat %>% group_by(barcode) %>% tally() %>% rename(count_EB = n) %>% 
            ...
            mutate(log2_fc = log2(prop_EB/prop_plasmid), comparing = "EB", prop = prop_EB) %>% as.data.frame()
        df2 = plasmid_cut %>% filter(barcode %in% df1$barcode) %>%
            mutate(log2_fc = log2(prop_ESC/prop_plasmid), comparing = "ESC", prop = prop_ESC) %>% as.data.frame()
        df = rbind(df1[,c("gene", "barcode", "log2_fc", "comparing", "prop", "prop_plasmid")],
                   df2[,c("gene", "barcode", "log2_fc", "comparing", "prop", "prop_plasmid")])
    } else {
        df1 = dat %>% group_by(barcode) %>% tally() %>% rename(count_EB = n) %>% 
...
saveRDS(res_orig, paste0(work_path, "/processing/pilot_screen_sgRNA/rep2_LogFC_EB_vs_plasmid_orig.rds"))
saveRDS(res, paste0(work_path, "/processing/pilot_screen_sgRNA/rep2_LogFC_EB_vs_plasmid.rds"))


###########################################
### Step-3: scatter plot between replicates

### scatter plot of rep1 i vs. rep2 i
### Spearman's rank correlation, rho = 0.478739, p-value = 0.006082

res_rep1 = readRDS(paste0(work_path, "/processing/pilot_screen_sgRNA/rep1_LogFC_EB_vs_plasmid.rds"))
res_rep2 = readRDS(paste0(work_path, "/processing/pilot_screen_sgRNA/rep2_LogFC_EB_vs_plasmid.rds"))


df = res_rep1[["rep1_EB21_CRISPRi_sorted_gRNA"]] %>% filter(comparing == "EB") %>% select(gene, rep1_i = mean_log2_fc) %>%
    left_join(res_rep2[["rep2_EB21_CRISPRi_sorted_gRNA"]] %>% filter(comparing == "EB") %>% select(gene, rep2_i = mean_log2_fc), by = "gene")
...
saveRDS(df, paste0(work_path, "/processing/pilot_screen_sgRNA/Compare_rep1_i_vs_rep2_i.rds"))

p = ggplot() +
    geom_point(data = df, aes(rep1_i, rep2_i)) + 
    geom_smooth(data = df, aes(rep1_i, rep2_i), method = "lm", se = FALSE) + 
    ggrepel::geom_text_repel(data = df, aes(rep1_i, rep2_i, label = gene, color = gene_group)) +
    theme_classic(base_size = 10) +
    theme(legend.position="none") +
    theme(plot.title = element_text(hjust = 0.5)) +
...
### scatter plot of rep2 cut vs. rep2 i
### Spearman's rank correlation, rho = 0.5381232, p-value = 0.001744

df = res_rep2[["rep2_EB21_CRISPRcut_sorted_gRNA"]] %>% filter(comparing == "EB") %>% select(gene, rep2_cut = mean_log2_fc) %>%
    left_join(res_rep2[["rep2_EB21_CRISPRi_sorted_gRNA"]] %>% filter(comparing == "EB") %>% select(gene, rep2_i = mean_log2_fc), by = "gene")
...
saveRDS(df, paste0(work_path, "/processing/pilot_screen_sgRNA/Compare_rep2_cut_vs_rep2_i.rds"))

p = ggplot() +
    geom_point(data = df, aes(rep2_cut, rep2_i)) + 
    geom_smooth(data = df, aes(rep2_cut, rep2_i), method = "lm", se = FALSE) + 
    ggrepel::geom_text_repel(data = df, aes(rep2_cut, rep2_i, label = gene, color = gene_group)) +
    theme_classic(base_size = 10) +
    theme(legend.position="none") +
    theme(plot.title = element_text(hjust = 0.5)) +
    scale_color_manual(values = pilot_gene_group_color_code) +
...
########################################################
### Step-4: showing a summary boxplot across four groups

res_orig = readRDS(paste0(work_path, "/processing/pilot_screen_sgRNA/rep2_LogFC_EB_vs_plasmid_orig.rds"))
...
for(run_id in run_list){
    ...
    p = ggplot(df, aes(gene_group, log2_fc, fill = gene_group)) + geom_boxplot(outlier.shape = NA) +
        geom_jitter(width = 0.2, size = 0.5) + facet_grid(.~comparing) +
