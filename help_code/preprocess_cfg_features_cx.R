...
.tenx_to_cds = function(pipeline_dirs) {
  ...
  lapply(pipeline_dirs, function(pipeline_dir) {
    ...
    base_path = file.path(pipeline_dir, "outs", "raw_feature_bc_matrix_mex")
    base_path.unaggregated = file.path(pipeline_dir, "outs", "raw_feature_bc_matrix")
    ...
    matrix_path   = file.path(base_path, "matrix.mtx.gz")
    genes_path    = file.path(base_path, "features.tsv.gz")
    barcodes_path = file.path(base_path, "barcodes.tsv.gz")
    
    cell_white_list_path = file.path(pipeline_dir, "outs", "cell_list_by_manual_filtering.txt")
    ...
    if( ! file.exists(cell_white_list_path) ) { stop(paste("Cell white list file not found:", cell_white_list_path)) }
    ...
    cell_white_list = read.table(cell_white_list_path, header=F, as.is=T)[,1]
    ...
    matrix = matrix[,colnames(matrix) %in% cell_white_list]
...
flag_low_size_factor_clusters = function(cds, cell_detection_threshold, log2_size_factor_threshold) {
  # Preprocess
  expressed_genes = row.names(fData(cds)[Matrix::rowSums(exprs(cds) > 0) > cell_detection_threshold,])
...
if (! args$aggregated) {
	aggregated_cds = .tenx_to_cds(sample_directories)
} else {
	...
	aggregated_cds = .tenx_to_cds(unique(sample_directories))
...
if (! is.null(args$barcode_enrichment_qc_plot)) {
    ...
    try(ggplot(combined_barcodes_plot_df, aes(x=proportion, y=log10(count_column))) +
        ...
        ggsave(args$barcode_enrichment_qc_plot, height=5, width=8), silent = T)
}
