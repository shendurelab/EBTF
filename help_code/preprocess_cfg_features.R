...
.tenx_to_cds = function(pipeline_dirs, genome="hg19", filtered=TRUE) {
  ...
  lapply(pipeline_dirs, function(pipeline_dir) {
    ...
    if (filtered) {
      base_path = file.path(pipeline_dir, "outs", "filtered_feature_bc_matrix_mex", genome)
      base_path.unaggregated = file.path(pipeline_dir, "outs", "filtered_feature_bc_matrix", genome)
      } else {
        base_path = file.path(pipeline_dir, "outs", "raw_feature_bc_matrix_mex", genome)
        base_path.unaggregated = file.path(pipeline_dir, "outs", "raw_feature_bc_matrix", genome)
      }
