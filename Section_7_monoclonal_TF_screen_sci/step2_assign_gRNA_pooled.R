...
#########################################################
### Assigning sgRNA to monoclonal EBs (pooled experiment)
### Chengxiang Qiu
### Feb-20, 2025
...
dat_BC = rbind(dat_BC_2, dat_BC_4)
...
try(ggplot() +
        ...
        ggsave(paste0(work_path, "/analysis/monoclonal_EB_TF_screen_sci/cell_to_mEB/BC_2D_hist_rep2.pdf"),
               height  = 3, 
               width = 5), silent = TRUE)

### After checking the 2D-hist, I decided to use log2_read_count >= 10 as cutoff to filter BCs
### n = 388 BCs are retained
...
saveRDS(bc_matrix, paste0(work_path, "/analysis/monoclonal_EB_TF_screen_sci/cell_to_mEB/bc_matrix_rep2.rds"))
...
print(sum(table(76.4) == 1)/length(unique(cell_BC$cell_id)))
### 80.8% cells are retained, and 69.2% cells have unique BC
...
saveRDS(cell_BC_assigned, paste0(work_path, "/analysis/monoclonal_EB_TF_screen_sci/cell_to_mEB/cell_clonotype_rep2.rds"))
print(nrow(cell_BC_assigned)/sum(pd$RT_group %in% c("GEx_mEB_ebTF_SR_filtered_pool11","GEx_mEB_ebTF_SR_gravity_pool11")))
### Finally, 16.5% of cells with transcriptome have been assigned to a distinct clonotype in rep2
### 42 clonotypes
### mean 86, median 75, sd 41, ranging 50 - 215 cells per clonotypes
...
cell_BC_assigned = readRDS(paste0(work_path, "/analysis/monoclonal_EB_TF_screen_sci/cell_to_mEB/cell_clonotype_rep2.rds"))
...
try(ggplot(df_x %>% filter(TF %in% c(target_i, "NONTARGETING"), celltype == celltype_i), aes(TF, pct, fill = TF)) + geom_boxplot(outlier.shape = NA) +
        ...
        theme(plot.title = element_text(hjust = 0.5)) +
        theme(axis.text.x = element_text(color="black"), axis.text.y = element_text(color="black")) +
        ggsave(paste0(work_path, "/analysis/monoclonal_EB_TF_screen_sci/cell_to_mEB/rep2_boxplot_", target_i, "_", gsub(" ","_",celltype_i), ".pdf"),
               height  = 5, 
               width = 3), silent = T)
...
x_1 = as.vector(df_x$pct[df_x$TF == target_i & df_x$celltype == celltype_i])
x_2 = as.vector(df_x$pct[df_x$TF == "NONTARGETING" & df_x$celltype == celltype_i])
wilcox.test(x_1, x_2) ### p = 0.001998
